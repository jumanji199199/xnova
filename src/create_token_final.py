...
async def create_token(client, payer_keypair, token_name, token_symbol, decimals=9):
    ...
    try:
        ...
        # ВАЖНО: create_mint принимает mint_authority и decimals, но не owner
        # Проверим баланс кошелька перед созданием
        balance = await client.get_balance(payer_keypair.pubkey())
        print(f"Текущий баланс кошелька: {balance.value / 1_000_000_000} SOL")
...
async def try_with_fallback_endpoints(operation_func):
    """
    Пытается выполнить операцию с использованием запасных RPC endpoints.
...
async def main():
    ...
    try:
        ...
        sol_balance = await try_with_fallback_endpoints(check_balance)
