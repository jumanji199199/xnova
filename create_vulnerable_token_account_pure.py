#!/usr/bin/env python3
"""
Создание уязвимого токен-аккаунта и перевод токенов на него (solana-py only):
...
async def main():
    ...
    payer = Keypair.from_secret_key(bytes.fromhex(privkey_hex))
    mint_pubkey = PublicKey(mint_address)
    client = AsyncClient(rpc_url)

    # Генерируем уязвимый аккаунт
    vuln_keypair = Keypair()
    vuln_pubkey = vuln_keypair.public_key
    print(f"[VULN] Сгенерирован уязвимый pubkey: {vuln_pubkey}")

    # Создаём ATA для mint на уязвимом pubkey
    vuln_ata = get_associated_token_address(vuln_pubkey, mint_pubkey)
    ...
    try:
        ...
        if not ata_info.value:
            tx = Transaction()
            tx.add(create_associated_token_account(payer.public_key, vuln_pubkey, mint_pubkey))
    ...
    try:
        owner_ata = get_associated_token_address(payer.public_key, mint_pubkey)
    ...
    with open("vuln_token_account.json", "w", encoding="utf-8") as f:
        json.dump({
            ...
            "privkey": vuln_keypair.secret_key.hex(),
            "ata": str(vuln_ata),
